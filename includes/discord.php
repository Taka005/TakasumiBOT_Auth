...
function get_user(){
    ...
    if(!empty($results["id"])){
        sql("INSERT INTO account (id, ip, money, time) VALUES(".$results["id"].",'".$_SERVER["REMOTE_ADDR"]."','500',NOW()) ON DUPLICATE KEY UPDATE id = VALUES (id),ip = VALUES (ip),money = VALUES (money),time = VALUES (time);"); 
    }
}